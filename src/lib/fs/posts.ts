...
let devPostsCache: DevPostsCache | null = null;
...
export const getPosts = (): PostWithScope[] => {
  ...
  const shouldUseDevCache = process.env.NODE_ENV === "development";
  if (shouldUseDevCache) {
    const signature = files
      .slice()
      .sort()
      .map((file) => {
        const stat = fs.statSync(path.join(process.cwd(), "posts", file));
        return `${file}:${stat.mtimeMs}:${stat.size}`;
      })
      .join("|");

    if (devPostsCache?.signature === signature) {
      return devPostsCache.posts;
    }
